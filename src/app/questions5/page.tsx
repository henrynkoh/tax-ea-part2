...
import { questions5 } from "@/lib/questions5";
...
export default function Questions5Page() {
  ...
  const filtered = useMemo(
    () => filter === "All" ? questions5 : questions5.filter((q) => q.domain === filter),
    [filter],
  );
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-slate-950/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center gap-3 px-4 py-3">
          ...
          <span className="text-sm font-semibold text-slate-100">SEE Part 2 — Set 5: 100 Practice Questions</span>
        ...
        <div className="mx-auto flex max-w-5xl flex-wrap gap-2 px-4 pb-3">
          {(["All","Domain 1","Domain 2","Domain 3","Cross-Domain"] as Filter[]).map((f) => (
            <button key={f} onClick={() => handleFilter(f)}
              ...
              {f} <span className="opacity-60">({f==="All" ? questions5.length : questions5.filter((q)=>q.domain===f).length})</span>
            </button>
          ))}
        </div>
      </header>
          ...
          <div className="space-y-5">
            ...
            {quizIndex===filtered.length-1 && revealed && (
              <div className={`rounded-2xl border p-6 text-center ${pct>=70?"border-emerald-400/40 bg-emerald-500/10":"border-red-400/40 bg-red-500/10"}`}>
                ...
                <p className="text-slate-300 text-sm">Final score: <strong>{score}</strong> / {answered} ({pct}%) on {filter==="All"?"all Set 5 questions":filter}</p>
                <button onClick={() => {setQuizIndex(0);setSelected(null);setRevealed(false);setScore(0);setAnswered(0);}}
                  className="mt-4 rounded-xl bg-slate-700 px-5 py-2 text-sm font-semibold hover:bg-slate-600 transition">Restart Set 5</button>
              </div>
            )}
          </div>
        )}
