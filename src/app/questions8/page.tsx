...
import { questions8 } from "@/lib/questions8";
...
export default function Questions8Page() {
  ...
  const filtered = useMemo(
    () => filter === "All" ? questions8 : questions8.filter((q) => q.domain === filter),
    [filter],
  );
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-slate-950/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center gap-3 px-4 py-3">
          ...
          <Link href="/questions9" className="rounded-lg border border-white/10 px-3 py-1.5 text-xs text-slate-300 transition hover:bg-slate-800">Set 9</Link>
          <span className="text-sm font-semibold text-slate-100">SEE Part 2 — Set 8: 100 Practice Questions</span>
            ...
            <div className="flex rounded-lg border border-white/10 overflow-hidden text-xs font-medium">
              {(["quiz","study"] as const).map((m) => (
                <button key={m} onClick={() => handleMode(m)}
                  className={`px-3 py-1.5 capitalize transition ${mode===m ? "bg-rose-500/25 text-rose-200" : "text-slate-400 hover:bg-slate-800"}`}>
        ...
        <div className="mx-auto flex max-w-5xl flex-wrap gap-2 px-4 pb-3">
          {(["All","Domain 1","Domain 2","Domain 3","Cross-Domain"] as Filter[]).map((f) => (
            <button key={f} onClick={() => handleFilter(f)}
              className={`rounded-full border px-3 py-1 text-xs font-medium transition ${filter===f ? "border-rose-400/60 bg-rose-500/20 text-rose-200" : "border-white/10 text-slate-400 hover:border-white/20 hover:text-slate-200"}`}>
              {f} <span className="opacity-60">({f==="All" ? questions8.length : questions8.filter((q)=>q.domain===f).length})</span>
            </button>
          ))}
        </div>
      </header>
          ...
          <div className="space-y-5">
            <div className="flex items-center gap-3 text-xs text-slate-400">
              ...
              <div className="flex-1 h-1.5 rounded-full bg-slate-800 overflow-hidden">
                <div className="h-full rounded-full bg-rose-500 transition-all" style={{width:`${((quizIndex+1)/filtered.length)*100}%`}} />
              </div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-slate-900 p-6 shadow-xl">
              ...
              <div className="space-y-3">
                {CHOICE_LABELS.map((c) => {
                  ...
                  } else if (isSel) style = "border-rose-400/60 bg-rose-500/15 text-rose-100";
                  return (
              ...
              {revealed && (
                <div className="mt-5 rounded-xl border border-rose-400/25 bg-rose-500/8 p-4">
                  <p className="text-xs font-semibold uppercase tracking-wide text-rose-300 mb-1">Explanation</p>
            ...
            <div className="flex gap-3">
              ...
              {!revealed ? (
                <button onClick={handleReveal} disabled={!selected}
                  className="flex-1 rounded-xl bg-rose-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-rose-500 disabled:opacity-30 disabled:cursor-not-allowed">Check Answer</button>
              ) : (
                <button onClick={handleNext} disabled={quizIndex===filtered.length-1}
                  className="flex-1 rounded-xl bg-pink-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-pink-500 disabled:opacity-30 disabled:cursor-not-allowed">Next Question →</button>
              )}
            </div>
            {quizIndex===filtered.length-1 && revealed && (
              <div className={`rounded-2xl border p-6 text-center ${pct>=70?"border-emerald-400/40 bg-emerald-500/10":"border-red-400/40 bg-red-500/10"}`}>
                ...
                <p className="text-slate-300 text-sm">Final score: <strong>{score}</strong> / {answered} ({pct}%) on {filter==="All"?"all Set 8 questions":filter}</p>
                <button onClick={() => {setQuizIndex(0);setSelected(null);setRevealed(false);setScore(0);setAnswered(0);}}
                  className="mt-4 rounded-xl bg-slate-700 px-5 py-2 text-sm font-semibold hover:bg-slate-600 transition">Restart Set 8</button>
              </div>
            )}
          </div>
        )}
              ...
              <div key={q.id} className="rounded-2xl border border-white/10 bg-slate-900 overflow-hidden">
                ...
                {studyOpen[q.id] && (
                  <div className="border-t border-white/10 px-5 pb-5 pt-4 space-y-3">
                    ...
                    <div className="rounded-xl border border-rose-400/20 bg-rose-500/8 p-3">
                      <p className="text-xs font-semibold uppercase tracking-wide text-rose-300 mb-1">Why</p>
